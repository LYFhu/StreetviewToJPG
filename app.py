...
def generate():
    ...
    if params is None:
        ...
        #pano = f.get_tile(params['panoid'], 3, 6, 3)
        ...
        compass = Image.open("static/compass.png").convert("RGBA")
        compass = compass.resize((int(images[0].width/20), int(images[0].height/10)), Image.LANCZOS)
        compass = compass.rotate(float(params['heading']))
        
        for i in range(4):
            images[i].paste(compass, (0, images[i].height-compass.height), mask=compass)
            compass = compass.rotate(90)
        
        '''Convert the images for download'''
